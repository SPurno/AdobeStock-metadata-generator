...
async function initEngine() {
  console.log('[ASMG] initEngine() called, tf=', typeof tf !== 'undefined' ? 'defined' : 'undefined', 'mobilenet=', typeof mobilenet !== 'undefined' ? 'defined' : 'undefined');
  
  // Show initial status
  window.parent.postMessage({ type: 'ENGINE_STATUS', message: 'Initializing...' }, '*');
  
  try {
    console.log('[ASMG] Starting loadTensorFlow()...');
    await loadTensorFlow();
    console.log('[ASMG] loadTensorFlow() succeeded, tfReady=', tfReady);
    
    window.parent.postMessage({ type: 'ENGINE_STATUS', message: 'TF ready, loading model...' }, '*');
    
    console.log('[ASMG] Starting loadMobileNetModel()...');
    await loadMobileNetModel();
    console.log('[ASMG] loadMobileNetModel() succeeded, modelReady=', modelReady);
    
    ready = true;
    console.log('[ASMG] Engine fully ready, sending ANALYSIS_ENGINE_READY');
    window.parent.postMessage({ type: 'ANALYSIS_ENGINE_READY', ready: true }, '*');
  } catch (error) {
    console.error('[ASMG] Engine init FAILED:', error.message);
    console.error('[ASMG] Stack:', error.stack);
    window.parent.postMessage({
      type: 'ANALYSIS_ENGINE_ERROR',
      error: error.message + ' (see browser console for details)'
    }, '*');
...
async function loadTensorFlow() {
  console.log('[ASMG] loadTensorFlow: checking if tf is available...');
  
  if (typeof tf !== 'undefined') {
    console.log('[ASMG] tf IS defined. Available backends:', Object.keys(tf).filter(k => k.includes('backend') || k.includes('Backend')).join(', '));
    console.log('[ASMG] Current backend:', tf.getBackend());
    
    // Always try CPU backend first to avoid CSP issues
    try {
      console.log('[ASMG] Setting backend to cpu...');
      await tf.setBackend('cpu');
      console.log('[ASMG] TensorFlow.js loaded, backend:', tf.getBackend());
      tfReady = true;
      return;
    } catch (e) {
      console.warn('[ASMG] CPU backend failed:', e.message);
      console.warn('[ASMG] Trying to use default backend instead...');
      // CPU not available, TF will auto-select WebGL or WASM
      try {
        const currentBackend = tf.getBackend();
        console.log('[ASMG] Current default backend:', currentBackend);
        tfReady = true;
        return;
      } catch (e2) {
        console.error('[ASMG] No backend available:', e2.message);
        throw new Error('No TF backend available: ' + e.message);
      }
    }
  }

  console.warn('[ASMG] tf is NOT defined yet, starting poll...');
  
  // Poll for tf to load (in case scripts are still loading)
  for (let i = 0; i < 100; i++) {
    if (typeof tf !== 'undefined') {
      console.log('[ASMG] tf became available on poll iteration', i);
      console.log('[ASMG] Current backend:', tf.getBackend());
      try {
        await tf.setBackend('cpu');
        console.log('[ASMG] TensorFlow.js loaded on poll, backend:', tf.getBackend());
      } catch (e) {
        console.warn('[ASMG] CPU backend not available on poll:', e.message);
      }
      tfReady = true;
      return;
    }
  ...
  throw new Error('TensorFlow.js failed to load after 10 seconds - tf.min.js may be missing from the extension package');
}
...
async function loadMobileNetModel() {
  if (modelLoadAttempted) {
    console.log('[ASMG] Model load already attempted, skipping');
    return;
  }
  modelLoadAttempted = true;

  console.log('[ASMG] Loading MobileNet model...');
  window.parent.postMessage({ type: 'ENGINE_STATUS', message: 'Loading MobileNet model from TF Hub...' }, '*');

  try {
    // Check if mobilenet global is available (from mobilenet.min.js)
    if (typeof mobilenet === 'undefined') {
      console.warn('[ASMG] mobilenet global not found, waiting for it to load...');
      // Wait a bit for the script to load
      for (let i = 0; i < 50; i++) {
        if (typeof mobilenet !== 'undefined') {
          console.log('[ASMG] mobilenet became available after', i * 200, 'ms');
          break;
        }
    ...
    if (typeof mobilenet === 'undefined') {
      console.error('[ASMG] mobilenet NEVER became available');
      throw new Error('MobileNet library not loaded - mobilenet.min.js may not be loading. Check that the library file exists in lib/vendor/');
    }

    console.log('[ASMG] mobilenet.load() starting, version=1, alpha=0.25');
    console.log('[ASMG] This will download ~4MB model from TF Hub...');
    
    window.parent.postMessage({ type: 'ENGINE_STATUS', message: 'Downloading MobileNet model (~4MB)...' }, '*');
    ...
    window.parent.postMessage({ type: 'ENGINE_STATUS', message: 'MobileNet model ready!' }, '*');
  } catch (error) {
    console.error('[ASMG] MobileNet load FAILED:', error.message);
    if (error.message && error.message.includes('fetch')) {
      console.error('[ASMG] This looks like a network error. Check that host_permissions include tfhub.dev and storage.googleapis.com');
    }
    window.parent.postMessage({
      type: 'MODEL_LOAD_ERROR',
      error: 'MobileNet failed: ' + error.message
    }, '*');
    throw error;
  }
}
